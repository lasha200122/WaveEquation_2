...
def I(x):
    return  np.sin(x) #np.sin(2*np.pi*x/3)


def V(x):
    return  0 #np.sin(2*np.pi*x/3) * np.pi*2 / 3
...
class WaveFinder:
    def __init__(self, path, L,n):
        ...
        self.C2 = np.sqrt(np.sqrt(sum([i ** 2for i in self.constants])))  #np.sqrt(abs(sum(np.array(self.constants))) / len(self.constants))
    ...
    def GetGrid(self):
        ...
        for key in self.frames.keys():
            ...
            X = []
            Y = []
            x = [i[1] - self.start[1] for i in coords if i[1] >= self.start[1]] #
            y = [self.start[0] - i[0] for i in coords if i[1] >= self.start[1]] #* self.scale * self.A
            ...
            NY = [x for _, x in sorted(zip(NX, NY))]
            NX = sorted(NX)
            print(NY)
            f_is = interp1d(np.array(NX) - self.start[1], self.start[0] - np.array(NY), kind="linear")
            yy = f_is(self.domain)
...
# dom1 = Real.domain
# Test = WaveSolver(I,V,f,2.0071413248719105, L, 0.01, 0.6388929266747607,T,Amax,ffmpeg,Func)
# dom2 = Test.domain
...
#     # diff = np.array(sol1[j]) - np.array(sol2[j])
#     # val = np.sqrt(sum([i ** 2 for i in diff]) * dx)
#     real = (dom1[1] - dom1[0]) * (sol1[j][0] + sol1[j][len(sol1[j]) - 1] + 2 * sum(sol1[j][1:len(sol1[j])-1])) / 2
#     test = (dom2[1] - dom2[0]) * (sol2[j][0] + sol2[j][len(sol2[j]) - 1] + 2 * sum(sol2[j][1:len(sol2[j]) - 1])) / 2
#     val = abs(real - test)
#     y.append(val)
#     x.append(j)
# plt.figure()
